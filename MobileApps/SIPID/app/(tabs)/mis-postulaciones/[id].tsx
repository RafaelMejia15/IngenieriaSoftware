...
export default function MisPostulacionesDetailScreen() {
  ...
  const renderUploadButton = (req: any) => {
    if (req.documento_subido) {
      return (
        <View className="bg-green-50 border border-green-200 p-3 rounded-xl mt-3 flex-row items-center justify-between">
          <View>
            <Text className="text-green-800 font-medium text-sm">Archivo subido</Text>
            <Text className="text-green-600 text-xs mt-0.5">{req.nombre_archivo_subido}</Text>
          </View>
    ...
    if (Platform.OS === 'web') {
      ...
      return (
        <View className="mt-3">
          ...
          />
          {selectedFile && (
            <View className="pt-2 px-4flex-row justify-end">
              <Button
                label={`Enviar`}
    ...
    return (
      <View className="mt-3 bg-orange-50 border border-orange-200 p-3 rounded-xl">
        <Text className="text-orange-800 text-sm font-medium">Sube tus archivos desde la versión Web</Text>
        <Text className="text-orange-600 text-xs mt-1">Por el momento, la subida nativa no está habilitada en la app móvil.</Text>
      </View>
    );
  };
  ...
  return (
    <SafeAreaView style={{ flex: 1 }} className="flex-1 bg-surface-50">
      <Header title="Detalle de Postulación" showHub={true} />
      <ScrollView className="flex-1" contentContainerStyle={{ flexGrow: 1 }}>
        <View className="p-4 md:p-8 max-w-4xl w-full mx-auto flex-grow">
          <View className="mb-8">
            <Text className="text-3xl font-extrabold text-surface-900 tracking-tight">{postulacion.convocatoria.nombre}</Text>
            <View className="flex-row items-center mt-3">
              <Badge label={postulacion.estado} variant="info" className="mr-3" />
              <Text className="text-surface-500 font-medium text-sm">Progreso: {postulacion.documentos_completos}/{postulacion.documentos_total} docs</Text>
            </View>
          </View>

          <Text className="text-surface-900 font-bold text-xl mb-4">Requisitos</Text>

          {postulacion.requisitos.map((req: any) => (
            <Card key={req.id_requisito} className="mb-4">
              <View className="flex-row items-start justify-between">
                <View className="flex-1 pr-4">
                  <Text className="text-surface-900 font-semibold text-base">{req.nombre}</Text>
                  <Text className="text-surface-500 text-xs mt-1 uppercase tracking-wider">{req.codigo} • {req.obligatorio ? 'Obligatorio' : 'Opcional'}</Text>
                </View>
              </View>
